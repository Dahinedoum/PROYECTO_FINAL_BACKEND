...
RUN groupadd -g 1000 food && \
	useradd -ms /bin/bash -u 1000 -g 1000 food && \
	mkdir -p /app && \
	chown food:food /app
...
COPY --chown=food:food package.json .
COPY --chown=food:food yarn.lock .

RUN yarn install --frozen-lockfile && yarn cache clean

COPY --chown=food:food src src/
